...
def analyze_and_clean():
    ...
    for column in analog_columns:
        ...
        if len(valid_data) == 0:
            ...
            continue
    df_live = df.drop(columns= dead_sensor)
    
    df_complete = df_live.dropna(subset = ['RPM', 'TPS', 'timestamp'])
    
    if len(df_complete) > 0:
        df_complete['datetime'] = pd.to_datetime(df_complete['timestamp'], unit='s')
